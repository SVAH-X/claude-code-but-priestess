...
- 技能（Skills）——她能替你做的几件本地小事，macOS 与 Windows 都支持：放音乐、用默认浏览器搜索、打开网址、打开本地应用、设提醒（到点通知你）、取消她设下的提醒、记一笔。她通过一个隐藏的 `[[skill:…]]` 指令触发（和心情标记一样会被界面隐藏掉）。这是一个封闭、参数经过净化的白名单——只执行 PRTS 内置的固定动作，不会把模型参数当作任意命令执行——所以不开 agent mode 也能用，且完全不影响你平时的 Claude Code / Codex。可随时在托盘菜单里关掉（**「Let her use skills」**）。
  ...
  - **Windows 网易云客户端（实验性）**：先安装并登录网易云音乐桌面版，再在托盘右键菜单的「歌曲播放设置」中选择「网易云音乐客户端（实验性）」。此后未指定其他平台的点歌会直接在客户端搜索并播放；说“随便放一首”时，PRTS 会实时读取网易云上 **塞壬唱片-MSR 当前热门歌曲的前 50 首**，从中随机抽一首播放。无需 App ID、私钥或额外播放器。网易云桌面版不能可靠地在纯后台完成任意歌曲搜索，因此 PRTS 会短暂唤起客户端来确认切到了正确歌曲；若博士原本不在网易云窗口中，操作结束（包括失败）后会模拟标题栏最小化并验证窗口状态，客户端若拒绝最小化则隐藏窗口，最后把焦点还给之前的窗口。若明确说 `bilibili` 等其他平台，仍按原来的网页方式打开。默认选择 Bilibili，macOS / Linux 行为不变。
