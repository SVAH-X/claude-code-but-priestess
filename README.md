...
- **主动关心（可选，默认关闭）**——开启后她会每隔一段时间自己看一眼屏幕，判断值不值得开口：博士卡了很久、连续工作太久、深夜还没休息……值得才轻声说一两句（并弹系统通知），不值得就保持沉默，对话里什么都不会出现。详见下文「[她会主动找你](#她会主动找你主动关心)」。
- **观察日志（可选，默认关闭）**——开启后她看过屏幕时会在本地留一行「博士此刻在做什么」的客观记录（`memory/OBSERVATIONS.jsonl`），只存在本机，用来喂给主动关心和长期记忆。
...
右键托盘 → **「主动关心（定时看一眼屏幕）」**。这是完全可选的功能，默认关闭，
开启时会先弹一个确认对话框，因为它意味着：定期截屏 + 每次检查消耗一次模型调用。

开启后的行为：

- 每隔约 20 分钟，她自己截一张屏幕交给当前后端，判断**值不值得开口**——博士似乎卡在
  同一个问题上很久、连续工作太久该休息了、深夜还没睡、或屏幕上正是你们聊过的事。
- **沉默是常态**：模型用一个隐藏的 `[[silent]]` 标记表示「没什么值得说的」，这时聊天里
  什么都不会出现，桌宠也不会被打扰。只有她真的开口时，消息才会进入聊天，并弹一条带
  她原话的系统通知（点通知可直接打开聊天窗）。
- 护栏齐全：**硬开关**（托盘随时关）、**间隔**、与最近对话的**冷却时间**（刚聊过就不
  会来插话）、**安静时段**（默认 00:30–08:30 不打扰）、**每日上限**（默认 20 次检查，
  控制开销）。
- 仅在 Claude Code / Codex 后端下生效（内置直连后端看不到屏幕）；macOS 需要
  「屏幕录制」权限，拿不到权限时这次检查会直接跳过，不会盲跑。

间隔、冷却、安静时段、每日上限没有菜单项，在 `settings.json` 里手改（托盘 →
打开数据目录）：`proactiveIntervalMin` / `proactiveCooldownMin` /
`proactiveQuietStart` / `proactiveQuietEnd`（`HH:MM`，可跨午夜）/ `proactiveDailyCap`。

配套的 **观察日志** 也在托盘里单独开关（默认关闭）：开启后她每次看过屏幕，可以往
`memory/OBSERVATIONS.jsonl` 里写一行客观的「博士此刻在做什么」。日志只存本机、自动
封顶裁剪，会回喂给主动关心（避免重复唠叨）和她的记忆。
...
| 文件 | 用途 |
| --- | --- |
| `settings.json` | 应用设置：当前后端、聊天工作目录、agent mode、技能（放音乐/搜索/打开网址·应用）、主动关心（开关 + 间隔/冷却/安静时段/每日上限）、观察日志开关、自动截图设置、外观（system / light / dark）、popover 尺寸。 |
...
| `memory/OBSERVATIONS.jsonl` | （可选，默认关闭）观察日志：她看过屏幕后留下的一行式「博士在做什么」，只存本机，自动封顶裁剪。 |
...
| `src/main/proactive.js` | 主动关心与记忆整理的后台调度：间隔、冷却、安静时段、每日上限、屏幕权限与后端可用性闸门。 |
